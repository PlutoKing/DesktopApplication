...
namespace LF.Mathematics
{
    ...
    public class LFMatrix : ICloneable
    {
        ...
        /// <returns></returns>
        public static LFMatrix VandermondeMatrix(double[] x, int m, int n)
        {
        ...
        /// <returns></returns>
        public LFVector Col(int index)
        {
            LFVector r = new LFVector(VectorType.Col, RowCount);
        ...
        /// <param name="index"></param>
        /// <returns></returns>
        public LFVector Row(int index)
        {
            LFVector r = new LFVector(VectorType.Row, ColCount);
        ...
        public static LFVector operator /(LFMatrix M, LFVector b)
        {
            ...
            if (m == n)
            {
                ...
                LFVector x = new LFVector(VectorType.Col, m);
                if (D != 0)
                {
                    // 计算Dj，把D的j列换成b
                    for (int j = 0; j < m; j++)
        ...
        public static LFVector operator *(LFMatrix M, LFVector v)
        {
            int m = M.RowCount;
            int n = M.ColCount;
            if (n == v.Length && v.Type == VectorType.Col)
            {
                LFVector r = new LFVector(VectorType.Col, m);
                for (int i = 0; i < m; i++)
                {
                    double sum = 0;
                    for (int j = 0; j < n; j++)
